...
public:
	...
	void StartEffect() 
	{
		bool bNegative = g_chaosController.IsEffectActive(EFFECT_INVERTG);
		int gravity = m_gravity * (bNegative ? -1 : 1);
...
CREATE_GRAVITY_EFFECT(EFFECT_LOWG, 200, "Low Gravity", lowg);

BEGIN_EFFECT(EFFECT_INVERTG, "Invert Gravity", invertg, true)
	void StartEffect() 
	{
		Msg("Setting sv_gravity to %i\n", -sv_gravity.GetInt());
...
public:
	...
	void StartEffect()
	{
		phys_timescale.SetValue(m_scale);
	}
	void StopEffect()
	{
...
CREATE_PHYSICS_SPEED_EFFECT(EFFECT_PHYS_SLOW, 0.25, "Slow Physics", phys_slow)

BEGIN_EFFECT(EFFECT_PULL_TO_PLAYER, "Black Hole", pull_to_player, true)
	void StartEffect()
	{
...
BEGIN_EFFECT(EFFECT_PUSH_FROM_PLAYER, "Repulsive", push_from_player, true)
	void StartEffect()
	{
		CBasePlayer* pPlayer = UTIL_GetLocalPlayer();
		if (pPlayer)
...
BEGIN_EFFECT(EFFECT_NO_MOVEMENT, "Stop", no_movement, true)
	void StartEffect()
	{
...
BEGIN_EFFECT(EFFECT_SUPER_MOVEMENT, "Super Speed", super_movement, true)
	void StartEffect()
	{
...
BEGIN_EFFECT(EFFECT_LOCK_VEHICLE, "Lock Vehicles", lock_vehicle, true)
	void DoOnVehicles(CPropVehicleDriveable *pVehicle)
	{
		variant_t emptyVariant;
		if (bStarting)
...
public:
	...
	void Think()
	{
		// Think thinks on the effect start, so we don't have a need for StartEffect
		DoNPCRels(m_disposition, false);
		if (m_disposition == D_LI) {
...
CREATE_NPC_RELATION_EFFECT(EFFECT_NPC_NEUTRAL, D_NU, "World of Apathy", npc_neutral)
CREATE_NPC_RELATION_EFFECT(EFFECT_NPC_FEAR, D_FR, "World of Fear", npc_fear)

DEFINE_EFFECT_SINGLEFIRE(EFFECT_TELEPORT_RANDOM, "Teleport to Random Place", teleport_random)
{
	CBasePlayer* pPlayer = UTIL_GetLocalPlayer();
...
CBaseEntity *ChaosSpawnVehicle(const char *className, int iSpawnType, const char *strModel, const char *strTargetname, const char *strScript)
{
	float flDistAway;
	float flExtraHeight;
	switch (iSpawnType)
...
DEFINE_EFFECT_SINGLEFIRE(EFFECT_SPAWN_VEHICLE, "Spawn Random Vehicle", spawn_vehicle)
{
	variant_t sVariant;
	int nRandom;
	...
	if (nRandom == 5)
	{
		...
		{
			CBaseEntity *pJalopy = ChaosSpawnVehicle("prop_vehicle_jeep", SPAWNTYPE_VEHICLE, "models/vehicle.mdl", "jalopy", "scripts/vehicles/jalopy.txt");
...
#if 0
// <>(EFFECT_ERROR, MAKE_STRING("(Error)"), EC_NONE, -1, ERROR_WEIGHT);
// <CEGravitySet>(EFFECT_ZEROG, MAKE_STRING("Zero Gravity"), EC_NONE, chaos_time_zerog.GetFloat(), chaos_prob_zerog.GetInt());
// <CEGravitySet>(EFFECT_SUPERG, MAKE_STRING("Super Gravity"), EC_NONE, chaos_time_superg.GetFloat(), chaos_prob_superg.GetInt());
// <CEGravitySet>(EFFECT_LOWG, MAKE_STRING("Low Gravity"), EC_NONE, chaos_time_lowg.GetFloat(), chaos_prob_lowg.GetInt());
// <CEGravitySet>(EFFECT_INVERTG, MAKE_STRING("Invert Gravity"), EC_NONE, chaos_time_invertg.GetFloat(), chaos_prob_invertg.GetInt());
// <CEPhysSpeedSet>(EFFECT_PHYS_PAUSE, MAKE_STRING("Pause Physics"), EC_NO_VEHICLE, chaos_time_phys_pause.GetFloat(), chaos_prob_phys_pause.GetInt());
// <CEPhysSpeedSet>(EFFECT_PHYS_FAST, MAKE_STRING("Fast Physics"), EC_NONE, chaos_time_phys_fast.GetFloat(), chaos_prob_phys_fast.GetInt());
// <CEPhysSpeedSet>(EFFECT_PHYS_SLOW, MAKE_STRING("Slow Physics"), EC_NONE, chaos_time_phys_slow.GetFloat(), chaos_prob_phys_slow.GetInt());
// <CEPullToPlayer>(EFFECT_PULL_TO_PLAYER, MAKE_STRING("Black Hole"), EC_NONE, chaos_time_pull_to_player.GetFloat(), chaos_prob_pull_to_player.GetInt());
// <CEPushFromPlayer>(EFFECT_PUSH_FROM_PLAYER, MAKE_STRING("Repulsive"), EC_NONE, chaos_time_push_from_player.GetFloat(), chaos_prob_push_from_player.GetInt());
// <CEStop>(EFFECT_NO_MOVEMENT, MAKE_STRING("Stop"), EC_NONE, chaos_time_no_movement.GetFloat(), chaos_prob_no_movement.GetInt());
// <CESuperMovement>(EFFECT_SUPER_MOVEMENT, MAKE_STRING("Super Speed"), EC_NONE, chaos_time_super_movement.GetFloat(), chaos_prob_super_movement.GetInt());
// <CELockVehicles>(EFFECT_LOCK_VEHICLE, MAKE_STRING("Lock Vehicles"), EC_BOAT | EC_BUGGY, chaos_time_lock_vehicle.GetFloat(), chaos_prob_lock_vehicle.GetInt());
// <CENPCRels>(EFFECT_NPC_HATE, MAKE_STRING("World of Hate"), EC_NONE, chaos_time_npc_hate.GetFloat(), chaos_prob_npc_hate.GetInt());
// <CENPCRels>(EFFECT_NPC_LIKE, MAKE_STRING("World of Love"), EC_NONE, chaos_time_npc_like.GetFloat(), chaos_prob_npc_like.GetInt());
// <CENPCRels>(EFFECT_NPC_NEUTRAL, MAKE_STRING("World of Apathy"), EC_NONE, chaos_time_npc_neutral.GetFloat(), chaos_prob_npc_neutral.GetInt());
// <CENPCRels>(EFFECT_NPC_FEAR, MAKE_STRING("World of Fear"), EC_NONE, chaos_time_npc_fear.GetFloat(), chaos_prob_npc_fear.GetInt());
// <>(EFFECT_TELEPORT_RANDOM, MAKE_STRING("Teleport to Random Place"), EC_PLAYER_TELEPORT, -1, chaos_prob_teleport_random.GetInt());//
// <CERandomVehicle>(EFFECT_SPAWN_VEHICLE, MAKE_STRING("Spawn Random Vehicle"), EC_NONE, -1, chaos_prob_spawn_vehicle.GetInt());
// <CERandomNPC>(EFFECT_SPAWN_NPC, MAKE_STRING("Spawn Random NPC"), EC_NONE, -1, chaos_prob_spawn_npc.GetInt());
// <CESwimInAir>(EFFECT_SWIM_IN_AIR, MAKE_STRING("Water World"), EC_PICKUPS, chaos_time_swim_in_air.GetFloat(), chaos_prob_swim_in_air.GetInt());
// <>(EFFECT_ONLY_DRAW_WORLD, MAKE_STRING("Where Are The Objects?"), EC_NONE, chaos_time_only_draw_world.GetFloat(), chaos_prob_only_draw_world.GetInt());
// <>(EFFECT_LOW_DETAIL, MAKE_STRING("Ultra Low Detail"), EC_NONE, chaos_time_low_detail.GetFloat(), chaos_prob_low_detail.GetInt());
// <CEPlayerBig>(EFFECT_PLAYER_BIG, MAKE_STRING("Player is Huge"), EC_NONE, chaos_time_player_big.GetFloat(), chaos_prob_player_big.GetInt());
// <CEPlayerSmall>(EFFECT_PLAYER_SMALL, MAKE_STRING("Player is Tiny"), EC_NONE, chaos_time_player_small.GetFloat(), chaos_prob_player_small.GetInt());
// <>(EFFECT_NO_MOUSE_HORIZONTAL, MAKE_STRING("No Looking Left/Right"), EC_NONE, chaos_time_no_mouse_horizontal.GetFloat(), chaos_prob_no_mouse_horizontal.GetInt());
// <>(EFFECT_NO_MOUSE_VERTICAL, MAKE_STRING("No Looking Up/Down"), EC_NONE, chaos_time_no_mouse_vertical.GetFloat(), chaos_prob_no_mouse_vertical.GetInt());
// <CESuperGrab>(EFFECT_SUPER_GRAB, MAKE_STRING("Didn't Skip Arm Day"), EC_NONE, chaos_time_super_grab.GetFloat(), chaos_prob_super_grab.GetInt());
// <CERandomWeaponGive>(EFFECT_GIVE_WEAPON, MAKE_STRING("Give Random Weapon"), EC_NONE, -1, chaos_prob_give_weapon.GetInt());
// <>(EFFECT_GIVE_ALL_WEAPONS, MAKE_STRING("Give All Weapons"), EC_NONE, -1, chaos_prob_give_all_weapons.GetInt());
// <CEWeaponsDrop>(EFFECT_DROP_WEAPONS, MAKE_STRING("Drop Weapons"), EC_HAS_WEAPON | EC_NEED_PHYSGUN, -1, chaos_prob_drop_weapons.GetInt());
// <>(EFFECT_NADE_GUNS, MAKE_STRING("Grenade Guns"), EC_NO_INVULN, chaos_time_nade_guns.GetFloat(), chaos_prob_nade_guns.GetFloat());
// <CEEarthquake>(EFFECT_EARTHQUAKE, MAKE_STRING("Wobbly"), EC_NONE, chaos_time_earthquake.GetFloat(), chaos_prob_earthquake.GetInt());
// <CE420Joke>(EFFECT_420_JOKE, MAKE_STRING("Funny Number"), EC_NO_INVULN, -1, chaos_prob_420_joke.GetInt());
// <CEZombieSpam>(EFFECT_ZOMBIE_SPAM, MAKE_STRING("Left 4 Dead"), EC_HAS_WEAPON, -1, chaos_prob_zombie_spam.GetInt());
// <>(EFFECT_EXPLODE_ON_DEATH, MAKE_STRING("NPCs Explode on Death"), EC_NONE, chaos_time_explode_on_death.GetFloat(), chaos_prob_explode_on_death.GetInt());
// <>(EFFECT_BULLET_TELEPORT, MAKE_STRING("Teleporter Bullets"), EC_NONE, chaos_time_bullet_teleport.GetFloat(), chaos_prob_bullet_teleport.GetInt());
// <CECredits>(EFFECT_CREDITS, MAKE_STRING("Credits"), EC_NONE, -1, chaos_prob_credits.GetInt());
// <CESuperhot>(EFFECT_SUPERHOT, MAKE_STRING("Superhot"), EC_NONE, chaos_time_superhot.GetFloat(), chaos_prob_superhot.GetInt());
// <CESupercold>(EFFECT_SUPERCOLD, MAKE_STRING("Supercold"), EC_NONE, chaos_time_supercold.GetFloat(), chaos_prob_supercold.GetInt());
// <CEBarrelShotgun>(EFFECT_BARREL_SHOTGUN, MAKE_STRING("Double Barrel Shotgun"), EC_NONE, chaos_time_barrel_shotgun.GetFloat(), chaos_prob_barrel_shotgun.GetInt());
// <CEQuickclip>(EFFECT_QUICKCLIP_ON, MAKE_STRING("Enable Quickclip"), EC_QC_OFF, chaos_time_quickclip_on.GetFloat(), chaos_prob_quickclip_on.GetInt());
// <CEQuickclip>(EFFECT_QUICKCLIP_OFF, MAKE_STRING("Disable Quickclip"), EC_NONE, chaos_time_quickclip_off.GetFloat(), chaos_prob_quickclip_off.GetInt());
// <CESolidTriggers>(EFFECT_SOLID_TRIGGERS, MAKE_STRING("Solid Triggers"), EC_NONE, chaos_time_solid_triggers.GetFloat(), chaos_prob_solid_triggers.GetInt());
// <CEColors>(EFFECT_RANDOM_COLORS, MAKE_STRING("Pretty Colors"), EC_NONE, chaos_time_random_colors.GetFloat(), chaos_prob_random_colors.GetInt());
// <CEBottle>(EFFECT_BEER_BOTTLE, MAKE_STRING("Beer I owed ya"), EC_NONE, -1, chaos_prob_beer_bottle.GetInt());
// <CEEvilNPC>(EFFECT_EVIL_ALYX, MAKE_STRING("Annoying Alyx"), EC_HAS_WEAPON, -1, chaos_prob_evil_alyx.GetInt());
// <CEEvilNPC>(EFFECT_EVIL_NORIKO, MAKE_STRING("Noriko, No!"), EC_NONE, -1, chaos_prob_evil_noriko.GetInt());
// <>(EFFECT_CANT_LEAVE_MAP, MAKE_STRING("Why So Rushed?"), EC_NONE, chaos_time_cant_leave_map.GetFloat(), chaos_prob_cant_leave_map.GetInt());
// <CEFloorIsLava>(EFFECT_FLOOR_IS_LAVA, MAKE_STRING("Floor Is Lava"), EC_NO_INVULN | EC_QC_OFF, chaos_time_floor_is_lava.GetFloat(), chaos_prob_floor_is_lava.GetInt());
// <CERandomSong>(EFFECT_PLAY_MUSIC, MAKE_STRING("Play Random Song"), EC_NONE, -1, chaos_prob_play_music.GetInt());
// <CEUseSpam>(EFFECT_USE_SPAM, MAKE_STRING("Grabby"), EC_NO_VEHICLE, chaos_time_use_spam.GetFloat(), chaos_prob_use_spam.GetInt());
// <>(EFFECT_ORTHO_CAM, MAKE_STRING("Orthographic Camera"), EC_NONE, chaos_time_ortho_cam.GetFloat(), chaos_prob_ortho_cam.GetInt());
// <CETreeSpam>(EFFECT_FOREST, MAKE_STRING("Surprise Reforestation!"), EC_NONE, chaos_time_forest.GetFloat(), chaos_prob_forest.GetInt());
// <CEMountedGun>(EFFECT_SPAWN_MOUNTED_GUN, MAKE_STRING("Spawn Mounted Gun"), EC_NONE, -1, chaos_prob_spawn_mounted_gun.GetInt());
// <CERestartLevel>(EFFECT_RESTART_LEVEL, MAKE_STRING("Restart Level"), EC_NONE, -1, chaos_prob_restart_level.GetInt());
// <CERemovePickups>(EFFECT_REMOVE_PICKUPS, MAKE_STRING("Remove All Pickups"), EC_PICKUPS | EC_NEED_PHYSGUN | EC_HAS_WEAPON, -1, chaos_prob_remove_pickups.GetInt());
// <CECloneNPCs>(EFFECT_CLONE_NPCS, MAKE_STRING("Suppression Field Hiccup"), EC_NONE, -1, chaos_prob_clone_npcs.GetInt());
// <CELockPVS>(EFFECT_LOCK_PVS, MAKE_STRING("Vision Machine Broke"), EC_NONE, chaos_time_lock_pvs.GetFloat(), chaos_prob_lock_pvs.GetInt());
// <CEDejaVu>(EFFECT_RELOAD_DEJA_VU, MAKE_STRING("Deja Vu?"), EC_PLAYER_TELEPORT, -1, chaos_prob_reload_deja_vu.GetInt());
// <CEBumpy>(EFFECT_BUMPY, MAKE_STRING("Bumpy Road"), EC_BUGGY, chaos_time_bumpy.GetFloat(), chaos_prob_bumpy.GetInt());
// <CENoBrake>(EFFECT_NO_BRAKE, MAKE_STRING("Broken Brakes"), EC_BUGGY, chaos_time_no_brake.GetFloat(), chaos_prob_no_brake.GetInt());
// <CEForceInOutCar>(EFFECT_FORCE_INOUT_CAR, MAKE_STRING("Force In/Out Vehicle"), EC_BUGGY | EC_BOAT | EC_PLAYER_TELEPORT,-1, chaos_prob_force_inout_car.GetInt());
// <CEWeaponRemove>(EFFECT_WEAPON_REMOVE, MAKE_STRING("Remove Random Weapon"), EC_HAS_WEAPON | EC_NEED_PHYSGUN, -1, chaos_prob_weapon_remove.GetInt());
// <>(EFFECT_INTERP_NPCS, MAKE_STRING("Laggy NPCs"), EC_NONE, chaos_time_interp_npcs.GetFloat(), chaos_prob_interp_npcs.GetInt());
// <CEPhysConvert>(EFFECT_PHYS_CONVERT, MAKE_STRING("Ran Out Of Glue"), EC_NONE, -1, chaos_prob_phys_convert.GetInt());
// <CEIncline>(EFFECT_INCLINE, MAKE_STRING("No Climbing"), EC_NONE, chaos_time_incline.GetFloat(), chaos_prob_incline.GetInt());
// <>(EFFECT_DISABLE_SAVE, MAKE_STRING("No Saving"), EC_NONE, chaos_time_disable_save.GetFloat(), chaos_prob_disable_save.GetInt());
// <>(EFFECT_NO_RELOAD, MAKE_STRING("No One Can Reload"), EC_HAS_WEAPON, chaos_time_no_reload.GetFloat(), chaos_prob_no_reload.GetInt());
// <>(EFFECT_NPC_TELEPORT, MAKE_STRING("You Teleport?"), EC_NONE, chaos_time_npc_teleport.GetFloat(), chaos_prob_npc_teleport.GetInt());
// <CEDeathWater>(EFFECT_DEATH_WATER, MAKE_STRING("Death Water"), EC_WATER, chaos_time_death_water.GetFloat(), chaos_prob_death_water.GetInt());
// <CERandomCC>(EFFECT_RANDOM_CC, MAKE_STRING("Color Incorrection"), EC_NONE, chaos_time_random_cc.GetFloat(), chaos_prob_random_cc.GetInt());
...
//Effects that can survive on Think or FastThink.
...
void CEPullToPlayer::LevelTransition()
{
	StartEffect();
}
...
void CEPushFromPlayer::LevelTransition()
{
	StartEffect();
}
...
void CESolidTriggers::LevelTransition()
{
	StartEffect();
}
...
void CECredits::LevelTransition()
{
	//if we reload, the credits visual disappears and as far as i know we can't put it back to where it was at time of load, so we don't bother restoring it
	//playing song 2 would make no sense without the visual
	m_bPlayedSecondSong = true;
...
void CELockPVS::LevelTransition()
{
	StartEffect();
}
...
void CEEarthquake::LevelTransition()
{
	StartEffect();
}
...
void CERandomCC::LevelTransition()
{
	StopEffect();
	DisplayGeneratedCC();
}
